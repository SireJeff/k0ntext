...
describe('validateInstallation', () => {
  ...
  test('passes with valid structure', async () => {
    ...
    fs.mkdirSync(path.join(testDir, '.claude', 'context'), { recursive: true });
    fs.mkdirSync(path.join(testDir, '.claude', 'research'), { recursive: true });
    fs.mkdirSync(path.join(testDir, '.claude', 'plans'), { recursive: true });

    fs.writeFileSync(path.join(testDir, 'CLAUDE.md'), '# CLAUDE.md');
    fs.writeFileSync(path.join(testDir, '.claude', 'README.md'), '# README');
    fs.writeFileSync(path.join(testDir, '.claude', 'settings.json'), '{}');
    fs.writeFileSync(
      path.join(testDir, '.claude', 'context', 'WORKFLOW_INDEX.md'),
      '# Workflow Index'
    );
  ...
  test('warns about remaining placeholders', async () => {
    ...
    fs.writeFileSync(path.join(testDir, 'CLAUDE.md'), '# {{PROJECT_NAME}}');
    fs.writeFileSync(path.join(testDir, '.claude', 'README.md'), '# README');
    fs.writeFileSync(path.join(testDir, '.claude', 'settings.json'), '{}');
    fs.writeFileSync(
      path.join(testDir, '.claude', 'context', 'WORKFLOW_INDEX.md'),
  ...
  test('fails for invalid JSON in settings', async () => {
    ...
    fs.writeFileSync(path.join(testDir, 'CLAUDE.md'), '# CLAUDE.md');
    fs.writeFileSync(path.join(testDir, '.claude', 'README.md'), '# README');
    fs.writeFileSync(path.join(testDir, '.claude', 'settings.json'), 'invalid json');
    fs.writeFileSync(
      path.join(testDir, '.claude', 'context', 'WORKFLOW_INDEX.md'),
      '# Workflow Index'
    );
...
describe('REQUIRED_STRUCTURE', () => {
  ...
  test('directories include .claude', () => {
    expect(REQUIRED_STRUCTURE.directories).toContain('.claude');
  });

  test('files include CLAUDE.md', () => {
    expect(REQUIRED_STRUCTURE.files).toContain('CLAUDE.md');
  });
});
